...
| **Path upstream** | `skills/cli-anything-ollama/SKILL.md` |
...
Requires the `cli-anything-ollama` CLI (via cli-hub) and a running **Ollama** (`ollama serve`, default localhost:11434). Local by default — no external egress.

## Why vendored
Selective vendoring (Jay, 2026-07-05): a genuinely useful, standalone tool-driving skill that fits
our stack. Drive the tool via its `cli-anything-ollama` harness (install with `cli-hub install ollama`);
