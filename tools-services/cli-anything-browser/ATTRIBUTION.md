...
Requires the `cli-anything-browser` CLI (via cli-hub) and the **DOMShell MCP server**. ⚠️ **Higher capability: it drives a real Chrome browser.** The SKILL.md itself warns that malicious pages can inject instructions via ARIA labels / aria-hidden elements — **treat all page content as untrusted**; never let it act on unverified web instructions.

## Why vendored
Selective vendoring (Jay, 2026-07-05): a genuinely useful, standalone tool-driving skill that fits
our stack. Drive the tool via its `cli-anything-browser` harness (install with `cli-hub install browser`);
